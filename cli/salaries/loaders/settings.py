...
def load_settings(ws: xw.Sheet) -> SettingsSchema:
    ...
    tu_monthly_deduction_rate = _load_cell(ws, SettingCell.TU_MONTHLY_DEDUCTION_RATE).quantize(frc)
    tu_pension_deduction_rate = _load_cell(ws, SettingCell.TU_PENSION_DEDUCTION_RATE).quantize(frc)
    healthy_leaves_rate = _load_cell(ws, SettingCell.HEALTHY_LEAVES_RATE).quantize(frc)
    healthy_leaves_based_on = _load_cell(ws, SettingCell.HEALTHY_LEAVES_BASED_ON, str)
    days_of_month = _load_cell(ws, SettingCell.DAYS_OF_MONTH).quantize(frc)
    leaves_without_pay_rate = _load_cell(ws, SettingCell.LEAVES_WITHOUT_PAY_RATE)
    leaves_without_pay_based_on = _load_cell(ws, SettingCell.LEAVES_WITHOUT_PAY_BASED_ON, str)
    overtime_rate = _load_cell(ws, SettingCell.OVERTIME_RATE).quantize(frc)
    overtime_based_on = _load_cell(ws, SettingCell.OVERTIME_BASED_ON, str)
    additional_leaves_rate = _load_cell(ws, SettingCell.ADDITIONAL_LEAVES_RATE).quantize(frc)
...
def _load_brackets(ws: xw.Sheet) -> list[BracketSchema]:
